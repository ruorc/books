...
export default function AboutPage() {
  ...
  const isFunctional = mode === MODES.FUNCTION;
  const engineTitle = isFunctional ? 'Functional (Hooks)' : 'Class Components';
  const EngineIcon = isFunctional ? Code2 : Terminal;
  const engineColorClass = isFunctional ? 'bg-sky-500/10 text-sky-500' : 'bg-amber-500/10 text-amber-500';
  ...
  const activeSystemThemeLabel = isDark ? THEME_LABELS[THEMES.DARK] : THEME_LABELS[THEMES.LIGHT];
  const themeValue = theme === THEMES.SYSTEM
      ...
      <section className="space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <AboutCard
            ...
            value={themeValue}
            icon={Palette}
            iconColorClass={isDark ? 'bg-purple-500/10 text-purple-500' : 'bg-orange-500/10 text-orange-500'}
